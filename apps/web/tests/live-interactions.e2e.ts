...
import { deriveReplayScript, parseSessionLog } from '@workspacealberta/wa-llm-replay'
import type { ReplayOverrideDoc } from '@workspacealberta/wa-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  it.skipIf(MODE === 'record')('cancels a hung stream deterministically via the readyFile marker', async () => {
    ...
    await expect.poll(
      () => page.getByRole('status').filter({ hasText: 'Elbows Up...' }).isVisible(),
      { timeout: 10_000 },
    ).toBe(true)
