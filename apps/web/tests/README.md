...
the deliberate composition divergences from `wa web` — are documented in
...
## Do not import `@workspacealberta/wa-client-*` here
...
`@workspacealberta/wa-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@workspacealberta/wa-client-web` and the boot-manifest
type from `@workspacealberta/wa-client-modules/client`: booting the real shell is what
...
`@workspacealberta/wa-client-runtime/client` because `client/runtime` is reachable
