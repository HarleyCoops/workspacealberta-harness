...
const REQUESTING_PACKAGE = '@workspacealberta/wa-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@workspacealberta/wa-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@workspacealberta/wa-client-ui-slots')).toBeNull()
    expect(resolveId('@workspacealberta/wa-client-ui-primitives')).toBeNull()
    expect(resolveId('@workspacealberta/wa-client-runtime/client')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@workspacealberta/wa-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@workspacealberta/wa-host-apiproxy/api')).toBeNull()
    expect(resolveId('@workspacealberta/wa-session/surface')).toBeNull()
    expect(resolveId('@workspacealberta/wa-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@workspacealberta/wa-goal/remote')).toBeNull()
    expect(() => resolveId('@workspacealberta/wa-goal')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @workspacealberta leak', () => {
    expect(() => resolveId('@workspacealberta/wa-agent')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@workspacealberta/wa-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@workspacealberta/wa-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits the parser-preloaded runtime for every dynamic bundle', () => {
    expect(resolveId('@workspacealberta/wa-client-runtime/client')).toBeNull()
    const withoutRequest = purityResolveId('@workspacealberta/wa-client-ui-goal')
    expect(withoutRequest('@workspacealberta/wa-client-runtime/client')).toBeNull()
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@workspacealberta/wa-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@workspacealberta/wa-client-runtime/client')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@workspacealberta/wa-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@workspacealberta/wa-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@workspacealberta/wa-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@workspacealberta/wa-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@workspacealberta/wa-client-fixture', { external: 'react' }))
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@workspacealberta/wa-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@workspacealberta/wa-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@workspacealberta/wa-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@workspacealberta/wa-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@workspacealberta/wa-client-connection/client.js.map')
