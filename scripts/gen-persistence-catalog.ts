...
const SESSION_PACKAGE = '@workspacealberta/wa-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@workspacealberta/wa-session/types'
 ...
 * top-level declaration (in `@workspacealberta/wa-session`) and any declaration
 * merge inside a `declare module '@workspacealberta/wa-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @workspacealberta/wa-session/known-event-types',
    ' */',
    '',
    '/**',
