...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@workspacealberta/wa-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@workspacealberta/wa-a/subpath'",
      "declare module '@workspacealberta/wa-client-ui-slots' {}",
      "const load = () => import('@workspacealberta/wa-b')",
      'export const view = <div />',
      "export type { Local } from './local.ts'",
    ].join('\n'))

    expect([...uses].sort()).toEqual([
      '@workspacealberta/wa-a',
      '@workspacealberta/wa-b',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@workspacealberta/wa-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@workspacealberta/wa-client-feature'],
      sourceUses: {
        '@workspacealberta/wa-agent': ['packages/client/runtime/src/index.ts'],
        '@workspacealberta/wa-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@workspacealberta/wa-agent': 'workspace:^',
        '@workspacealberta/wa-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@workspacealberta/wa-agent': 'workspace:^',
        '@workspacealberta/wa-client-feature': 'workspace:^',
        '@workspacealberta/wa-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@workspacealberta/wa-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@workspacealberta/wa-agent': 'workspace:^' },
  ...
  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@workspacealberta/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@workspacealberta/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@workspacealberta/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@workspacealberta/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
        react: ['packages/client/web/src/seed.ts'],
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@workspacealberta/cordis-plugin-loader': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@workspacealberta/wa-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@workspacealberta/wa-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@workspacealberta/wa-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@workspacealberta/wa-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@workspacealberta/wa-client-slots/client'] })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@workspacealberta/wa-client-ui', '@workspacealberta/wa-missing', '@workspacealberta/wa-missing'],
      inject: ['', '@workspacealberta/wa-a', '@workspacealberta/wa-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@workspacealberta/wa-client-b'],
      inject: ['@workspacealberta/wa-client-b'],
    })
    const b = declaration('b', {
      external: ['@workspacealberta/wa-client-a'],
      inject: ['@workspacealberta/wa-client-a'],
    })
...
describe('manifest declarations', () => {
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@workspacealberta/wa-client-feature', '@workspacealberta/wa-missing'],
      inject: ['', '@workspacealberta/wa-agent', '@workspacealberta/wa-agent'],
      sourceUses: {
        '@workspacealberta/wa-agent': ['packages/client/feature/src/index.ts'],
        '@workspacealberta/wa-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@workspacealberta/wa-agent': 'workspace:*',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@workspacealberta/wa-missing'],
      inject: ['@workspacealberta/wa-agent'],
    })
