...
/** Drop the `@workspacealberta/wa-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@workspacealberta/wa-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @workspacealberta/wa-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
