...
const RELEASE_ENTRY_PACKAGE = '@workspacealberta/wa'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@workspacealberta/')) {
        throw new Error(`${manifestPath} must name an @workspacealberta package`)
      }
      if (name === '@workspacealberta/wa-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@workspacealberta/wa/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@workspacealberta/') || name === '@workspacealberta/wa-root')) {
