...
const MERGE_HEAD = /declare module ['"](?:@workspacealberta\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@workspacealberta/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@workspacealberta/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@workspacealberta/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@workspacealberta/cordis'` Context merge
...
/**
 * Every event name a `declare module '@workspacealberta/cordis'` Events merge declares in one
