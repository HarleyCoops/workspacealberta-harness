...
The public package family belongs to the `@workspacealberta` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@workspacealberta/node-addon-landlock-run
@workspacealberta/node-addon-landlock-run-<platform>
```
