...
| Package | Role |
|---|---|
| `@workspacealberta/wa-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@workspacealberta/wa-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@workspacealberta/wa-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@workspacealberta/wa-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@workspacealberta/wa-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@workspacealberta/wa-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@workspacealberta/wa-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
