...
The old Agent Note already deferred a separate `@workspacealberta/wa-fs-observation-policy` package. This decision builds that layer and keeps `ctx.fs` close to fsspec-style storage primitives (`info`/`cat`/`open`), without turning it into full fsspec.
...
`@workspacealberta/wa-fs` shrinks to provider text IO plus guarded text mutation:
