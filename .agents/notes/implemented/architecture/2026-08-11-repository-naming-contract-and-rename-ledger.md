...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@workspacealberta/wa-sdk-client`, `@workspacealberta/wa-sdk-protocol`, and the wire identity `deepseek-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. DeepSeek Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
|---|---|---|
| `@workspacealberta/wa-jsonrpc` | `@workspacealberta/wa-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@workspacealberta/wa-sdk-client`, `@workspacealberta/wa-sdk-protocol`, and `deepseek-harness-sdk-runtime`. Exclude `@workspacealberta/create-sdk`, `@workspacealberta/wa-scripts`, `@workspacealberta/wa-helper`, and `@workspacealberta/wa-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@workspacealberta/wa-bash`, `ctx.bash` | `@workspacealberta/wa-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@workspacealberta/wa-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@workspacealberta/wa-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@workspacealberta/wa-pty`, `ctx.pty`, `PtyService` | `@workspacealberta/wa-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@workspacealberta/wa-pty-local`, `LocalPtyBackend` | `@workspacealberta/wa-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@workspacealberta/wa-tool-pty` | `@workspacealberta/wa-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@workspacealberta/wa-lsp-local` | `@workspacealberta/wa-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@workspacealberta/wa-tasks`, `ctx.tasks`, `TaskService` | `@workspacealberta/wa-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@workspacealberta/wa-tasks-local`, `LocalTaskService` | `@workspacealberta/wa-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@workspacealberta/wa-tool-tasks` | `@workspacealberta/wa-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@workspacealberta/wa-client-ui-task`, `client/ui-task/` | `@workspacealberta/wa-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@workspacealberta/wa-client-ui-slash`, `ui-slash/` | `@workspacealberta/wa-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@workspacealberta/wa-agent-tool-mode`, plugin `tool-mode` | `@workspacealberta/wa-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@workspacealberta/wa-permission`, `ctx.permission`, `PermissionService` | `@workspacealberta/wa-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@workspacealberta/wa-client-ui-permission` | `@workspacealberta/wa-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@workspacealberta/wa-type-meta` | `typert/protocol/`, `@workspacealberta/wa-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@workspacealberta/wa-tools` and `ctx.tools`. Keep `@workspacealberta/wa-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@workspacealberta/wa-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@workspacealberta/wa-workspace-context`, `context/workspace-context/` | `@workspacealberta/wa-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `session/user-id/`, `@workspacealberta/wa-user-id` | `identity/anonymous-user-id/`, `@workspacealberta/wa-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@workspacealberta/wa-environment` | `util/launch-environment/`, `@workspacealberta/wa-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
...
| Former | Current | Reason |
|---|---|---|
| `@workspacealberta/wa-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@workspacealberta/wa-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@workspacealberta/wa-workflow` and workflow events and tools. |
| `@workspacealberta/wa-workflow-workerthread`, `WorkerWorkflowEngine` | `@workspacealberta/wa-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@workspacealberta/wa-goal-session`, `goal/goal-session/` | `@workspacealberta/wa-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@workspacealberta/wa-compact`, `ctx.compact`, `CompactService` | `@workspacealberta/wa-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@workspacealberta/wa-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@workspacealberta/wa-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@workspacealberta/wa-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@workspacealberta/wa-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@workspacealberta/wa-settings-local`, `SettingsLocal` | `@workspacealberta/wa-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@workspacealberta/wa-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@workspacealberta/wa-session-title-first-message-llm`, cadence `first-message` | `@workspacealberta/wa-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@workspacealberta/wa-session-title-all-messages-llm`, cadence `all-user-messages` | `@workspacealberta/wa-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@workspacealberta/wa-code-runtime-worker`, `WorkerCodeRuntime` | `@workspacealberta/wa-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@workspacealberta/wa-fs-policy` | `@workspacealberta/wa-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@workspacealberta/wa-skill-local`, `LocalSkillProvider`, provider id `local` | `@workspacealberta/wa-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@workspacealberta/wa-subagent-spawn`, `SpawnProvider` | `@workspacealberta/wa-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@workspacealberta/wa-subagent-fork`, `ForkProvider` | `@workspacealberta/wa-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@workspacealberta/wa-subagent-inprocess`, `subagent-inprocess/` | `@workspacealberta/wa-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
...
| `@workspacealberta/wa-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@workspacealberta/wa-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@workspacealberta/wa-subagent-dsh-sdk`, its provider id `dsh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@workspacealberta/wa-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@workspacealberta/wa-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@workspacealberta/wa-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@workspacealberta/wa-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@workspacealberta/wa-timeout-policy` | `@workspacealberta/wa-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@workspacealberta/wa-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@workspacealberta/wa-paths` | `util/home-paths/`, `@workspacealberta/wa-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@workspacealberta/wa-retention` | `util/output-retention/`, `@workspacealberta/wa-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@workspacealberta/wa-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@workspacealberta/wa-frontend-static` | `@workspacealberta/wa-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@workspacealberta/wa-jsonrpc-demo` | `@workspacealberta/wa-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@workspacealberta/wa-frontend` | `@workspacealberta/wa-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |
...
| `@workspacealberta/wa-client-ui-model` | `@workspacealberta/wa-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@workspacealberta/wa-client-ui-models` | `@workspacealberta/wa-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@workspacealberta/wa-client-ui-plugin-config`, `client/ui-plugin-config/` | `@workspacealberta/wa-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@workspacealberta/wa-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@workspacealberta/wa-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@workspacealberta/wa-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@workspacealberta/wa-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@workspacealberta/wa-client-ui-question` | `@workspacealberta/wa-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@workspacealberta/wa-client-ui-command`, `ui-command/` | `@workspacealberta/wa-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@workspacealberta/wa-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@workspacealberta/wa-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@workspacealberta/wa-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@workspacealberta/wa-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `@workspacealberta/wa-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@workspacealberta/wa-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
