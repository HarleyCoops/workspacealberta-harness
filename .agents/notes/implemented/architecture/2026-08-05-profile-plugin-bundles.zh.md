...
默认 Profile 模板使用的组合包是 `@workspacealberta/wa-base`（共享核心配置行）、`@workspacealberta/wa-web-app`（浏览器 Host 配置行与 Web 运行时粘合层）和 `@workspacealberta/wa-headless`（直接叠加在 base 上且不含 web-app 的一次性 runner）。通用的 `wa --profile <name>` 把剩余参数交给该 profile 的命令行启动行：Web 持有自己的 flag 家族，headless 则持有任务位置参数。patch overlay 使用启动器持有的 `--patch`。`wa plugin --profile <name> <args...>` 是一层薄薄的 pnpm 转发器，负责初始化 profile，并依据已安装包的组合包声明调和 `dsh.profile.bundles`；没有组合包声明的包保持为普通依赖。[Headless 作为直接 core 入口](2026-08-09-headless-direct-core-entry-point.md)负责 headless 组合约定。
