...
1. `@workspacealberta/wa-fs` (`packages/fs/fs`) owns the abstract `ctx.fs` service, the filesystem vocabulary types, and the `fs/*` policy event vocabulary.
2. `@workspacealberta/wa-fs-local` (`packages/fs/fs-local`) provides the first implementation, backed by the local filesystem.
3. `@workspacealberta/wa-tool-fs` (`packages/fs/tool-fs`) provides the model-facing `read`, `write`, and `edit` tools over `ctx.fs`, and is the executor that dispatches the `fs/*` events.
...
```text
@workspacealberta/wa-tool-fs  --depends on-->  @workspacealberta/wa-fs  <--depends on--  @workspacealberta/wa-fs-local
        consumer                                interface                         implementation
```

`@workspacealberta/wa-fs` depends only on `cordis` plus the repo-wide `HarnessError` base from `@workspacealberta/wa-llm`. It declares the `ctx.fs` key, the abstract `FileSystem` service, the vocabulary types shared by backends and consumers, the filesystem error vocabulary, and the `fs/*` policy event vocabulary. It carries no observed-state store and no owner-derivation shape; the events pass an opaque `object` actor that the provider never reads, and the `dsh-fs-observation-policy` plugin owns the owner-derivation shape and the observed-state store on top of those events.

`@workspacealberta/wa-fs-local` depends on `@workspacealberta/wa-fs` and `cordis`. It subclasses `FileSystem`, registers itself as `ctx.fs`, owns local-backend configuration such as the base directory, and contains all direct `node:fs` / `node:path` access. It holds no observed-state store — freshness is a version token the backend mints and the policy plugin records.

`@workspacealberta/wa-tool-fs` depends on `@workspacealberta/wa-fs`, `@workspacealberta/wa-tools`, `@workspacealberta/wa-system-prompt`, and `cordis`. It registers model-facing tools and prompt sections. It must not import `node:fs`, `node:path`, or `@workspacealberta/wa-fs-local`; filesystem execution always goes through `ctx.fs`. If the implementation needs concrete agent or session helper types, those dependencies belong in `tool-fs`; they must not leak back into `dsh-fs`.
...
`@workspacealberta/wa-fs` owns a semantic filesystem service. It is higher-level than `readFile` / `writeFile` so `tool-fs` does not reimplement path resolution, versioning, text decoding, binary rejection, pagination, atomic replacement, symlink behavior, or literal edit semantics.
...
`@workspacealberta/wa-tool-fs` is the model-facing consumer. It owns tool names, JSON schemas, argument validation at the model boundary, prompt sections, and result formatting. It does not own filesystem execution.
