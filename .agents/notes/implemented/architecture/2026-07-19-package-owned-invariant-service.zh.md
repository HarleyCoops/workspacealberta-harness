...
`@workspacealberta/wa-invariants` 是与产品无关的 Cordis 服务插件，注册 `ctx.invariants`。它只负责配置、注册唯一性、子 fiber 生命周期和带包归属的失败；不导入 session、agent、scope 或 agent-loop 包，也不包含这些包的检查。
...
| 伴随入口 | 注册名 | 所属检查 |
|---|---|---|
| `@workspacealberta/wa-session/invariant` | `@workspacealberta/wa-session` | 会话序列、轮次/步骤包围关系和同一步骤的调用/结果轨迹 |
| `@workspacealberta/wa-agent/invariant` | `@workspacealberta/wa-agent` | agent 状态转换 |
| `@workspacealberta/wa-scope/invariant` | `@workspacealberta/wa-scope` | 作用域事件载体的存在性与主体一致性 |
| `@workspacealberta/wa-agent-loop/invariant` | `@workspacealberta/wa-agent-loop` | 模型请求重建 |
