...
1. `@workspacealberta/wa-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@workspacealberta/wa-web-search-exa`, `@workspacealberta/wa-web-search-perplexity`, `@workspacealberta/wa-web-search-deepseek`, and `@workspacealberta/wa-web-fetch-http`.
3. `@workspacealberta/wa-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@workspacealberta/wa-tool-web  --depends on-->  @workspacealberta/wa-web  <--depends on--  @workspacealberta/wa-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @workspacealberta/wa-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @workspacealberta/wa-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @workspacealberta/wa-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@workspacealberta/wa-web-search-exa"] -->|registerSearchProvider| web["@workspacealberta/wa-web / ctx.web"]
  perplexity["@workspacealberta/wa-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@workspacealberta/wa-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@workspacealberta/wa-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@workspacealberta/wa-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@workspacealberta/wa-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@workspacealberta/wa-tool-web` depends on `@workspacealberta/wa-web`, `@workspacealberta/wa-tools`, `@workspacealberta/wa-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@workspacealberta/wa-web'
...
```yaml
- id: web
  name: '@workspacealberta/wa-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@workspacealberta/wa-web-search-exa'

- id: web-search-perplexity
  name: '@workspacealberta/wa-web-search-perplexity'

- id: web-search-deepseek
  name: '@workspacealberta/wa-web-search-deepseek'

- id: web-fetch-http
  name: '@workspacealberta/wa-web-fetch-http'
