...
| Package | Role |
|---|---|
| `@workspacealberta/wa-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@workspacealberta/wa-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@workspacealberta/wa-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@workspacealberta/wa-spill-local'

- id: spill-policy
  name: '@workspacealberta/wa-spill-policy'
  config:
...
- `@workspacealberta/wa-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@workspacealberta/wa-spill` owns saving final text and returning a locator plus retrieval hint.
- `@workspacealberta/wa-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
