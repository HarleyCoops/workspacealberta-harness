...
The default Profile templates use `@workspacealberta/wa-base` (shared core rows), `@workspacealberta/wa-web-app` (browser Host rows and Web runtime glue), and `@workspacealberta/wa-headless` (a direct one-shot runner over base, without web-app). Generic `wa --profile <name>` hands its remaining arguments to that profile's command-line startup row: Web owns its flag family, while headless owns its task positional. Patch overlays use launcher-owned `--patch`. `wa plugin --profile <name> <args...>` is a thin pnpm forwarder that initializes the profile and reconciles `dsh.profile.bundles` with installed bundle declarations; a package without a bundle declaration remains a plain dependency. [Headless as a direct core entry point](2026-08-09-headless-direct-core-entry-point.md) owns the headless composition contract.
