...
import { LlmAdapter, type GenerateOptions, type StreamChunk } from '@workspacealberta/wa-llm'
...
```ts
import { CallId, type StreamChunk } from '@workspacealberta/wa-llm'
...
- id: agent-loop
  name: '@workspacealberta/wa-agent-loop'
  config:
    agents:
      - id: main
