...
These three **default-off reference configurations** connect one memory system to DSH through [`@workspacealberta/wa-mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.
...
```sh
wa web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
wa web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
wa web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
