...
```
@workspacealberta/cordis-plugin-timer  timer service (writes nothing to stdout)
@workspacealberta/wa-llm              abstract LLM service + content-block vocabulary
@workspacealberta/wa-session          event-sourced session log + store
@workspacealberta/wa-session-title    log-backed title service + deterministic fallback
@workspacealberta/wa-system-prompt    prompt-section + tool-schema assembly
@workspacealberta/wa-tools            registry + guarded pre/around/post/final-result pipeline
@workspacealberta/wa-skill            skill provider registry
@workspacealberta/wa-skill-filesystem      local filesystem skill provider
@workspacealberta/wa-agent            agent registry + initiator scope + agent/* events
@workspacealberta/wa-goal             optional persisted same-session goal domain
@workspacealberta/wa-tool-goal        optional model-facing goal controls
@workspacealberta/wa-goal-round-driver     optional same-session goal-round driver
@workspacealberta/wa-llm-retry        provider-routed request retry policy
@workspacealberta/wa-jobs-local      generic background-job registry
@workspacealberta/wa-invariants       configurable invariant registry service
@workspacealberta/wa-session/invariant
@workspacealberta/wa-agent/invariant
@workspacealberta/wa-scope/invariant
@workspacealberta/wa-agent-loop/invariant
                                  package-owned relational checks
@workspacealberta/wa-tool-bash        the model-facing bash schema (unless toolBash=false)
@workspacealberta/wa-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@workspacealberta/wa-tool-skill       session-prefix skill catalog + model-facing loader schema
@workspacealberta/wa-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@workspacealberta/wa-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@workspacealberta/wa-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@workspacealberta/wa-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
