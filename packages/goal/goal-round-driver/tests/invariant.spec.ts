...
import * as GoalSessionInvariant from '@workspacealberta/wa-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@workspacealberta/wa-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@workspacealberta/wa-invariants'
import SessionStore, { SessionId, type Session } from '@workspacealberta/wa-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@workspacealberta/wa-goal-round-driver',
    }))
  })
