...
/**
 * Keyless built-artifact smoke: plain Node imports `@workspacealberta/wa-lsp` and
 * `@workspacealberta/wa-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@workspacealberta/cordis')
      const { default: Lsp } = await import('@workspacealberta/wa-lsp')
      const LspLocal = await import('@workspacealberta/wa-lsp-stdio')
      const { default: LocalFileSystem } = await import('@workspacealberta/wa-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@workspacealberta/wa-subprocess-local')
