...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@workspacealberta/wa-session-persistence-jsonl', JsonlSessionPersistence],
    ['@workspacealberta/wa-storage', Storage],
    ['@workspacealberta/wa-storage-json', StorageJson],
    ['@workspacealberta/wa-storage-domain', StorageDomain],
    ['@workspacealberta/wa-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@workspacealberta/wa-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@workspacealberta/wa-storage'",
      "- name: '@workspacealberta/wa-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@workspacealberta/wa-storage-domain'",
      '  config:',
      '    backend: json',
