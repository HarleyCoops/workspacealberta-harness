 ...
 *
 * @module @workspacealberta/wa-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@workspacealberta/wa-output-retention'
import type { RetainedItems } from '@workspacealberta/wa-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@workspacealberta/wa-subprocess'
 ...
 * `@workspacealberta/wa-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@workspacealberta/wa-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@workspacealberta/wa-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@workspacealberta/wa-tool-fs-search/presentation} `grepSearchMeta`)
