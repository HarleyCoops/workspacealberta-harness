 ...
 * `@workspacealberta/wa-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@workspacealberta/wa-shell`.
 *
 * @module @workspacealberta/wa-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@workspacealberta/cordis'
import z from '@workspacealberta/schemastery'
import { defineTool, TOOL_ABORTED } from '@workspacealberta/wa-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@workspacealberta/wa-tools'
...
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@workspacealberta/wa-sandbox'
import type { SandboxPolicyService } from '@workspacealberta/wa-sandbox-policy'
import type { ShellRunResult } from '@workspacealberta/wa-shell'
import { parseExitStatus } from '@workspacealberta/wa-shell'
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @workspacealberta/wa-jobs and @workspacealberta/wa-tool-jobs')
        }
