...
import SessionStore, { type Session, type SessionEvent } from '@workspacealberta/wa-session'
import InvariantRegistry, { InvariantError } from '@workspacealberta/wa-invariants'
import * as SandboxPolicyInvariant from '@workspacealberta/wa-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@workspacealberta/wa-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@workspacealberta/wa-sandbox-policy',
    })
  })
})
