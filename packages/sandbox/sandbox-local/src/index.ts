 ...
 * @module @workspacealberta/wa-sandbox-local
 */
...
} from '@workspacealberta/node-addon-landlock-run'
import { Context } from '@workspacealberta/cordis'
import z from '@workspacealberta/schemastery'
import { assertNever } from '@workspacealberta/wa-llm'
import { SandboxProvider, SandboxUnavailableError } from '@workspacealberta/wa-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@workspacealberta/wa-sandbox'
import type { SessionId } from '@workspacealberta/wa-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@workspacealberta/wa-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@workspacealberta/wa-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@workspacealberta/wa-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@workspacealberta/wa-sandbox-windows-acl/src/runner.ts'))
