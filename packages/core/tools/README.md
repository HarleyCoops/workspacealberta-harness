...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@workspacealberta/wa-tool-call-timeout-policy` wrapper.
