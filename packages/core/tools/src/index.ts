 ...
 * execution pipeline.
 * @module @workspacealberta/wa-tools
 */

import { Context, Service } from '@workspacealberta/cordis'
import z from '@workspacealberta/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@workspacealberta/wa-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@workspacealberta/wa-scope'
import type { CallId, ContentBlock, ToolSchema } from '@workspacealberta/wa-llm'
import { assertNever, deepFreeze, HarnessError } from '@workspacealberta/wa-llm'
import type { Agent } from '@workspacealberta/wa-agent'
import { snapshotJsonValue } from '@workspacealberta/wa-session'
import type { JsonValue, UserMessage } from '@workspacealberta/wa-session'
import type { ToolProviderResult } from '@workspacealberta/wa-system-prompt'
import type { CodeRuntime } from '@workspacealberta/wa-code-runtime'
...
export type { JsonValue } from '@workspacealberta/wa-session'
...
// lives in its own UI-facing module; re-export it so `@workspacealberta/wa-tools`
...
declare module '@workspacealberta/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@workspacealberta/wa-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@workspacealberta/wa-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@workspacealberta/wa-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@workspacealberta/wa-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@workspacealberta/wa-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @workspacealberta/wa-code-runtime-worker-thread) or set tools mode to "native"`)
    }
