...
describe('gen-tool-catalog collectToolCatalog', () => {
  ...
  it('attributes each harvested tool with its registering plugin source', async () => {
    const catalog = await collectToolCatalog()
    const bash = catalog.find(entry => entry.pkg === '@workspacealberta/wa-tool-bash')
    expect(bash?.sources.bash).toBe('packages/shell/tool-bash/src/index.ts')
    const control = catalog.find(entry => entry.pkg === '@workspacealberta/wa-tool-subagent-control')
  ...
  it('harvests search tools without depending on the generator process PATH', async () => {
    ...
    try {
      process.env.PATH = ''
      const catalog = await collectToolCatalog()
      const search = catalog.find(entry => entry.pkg === '@workspacealberta/wa-tool-fs-search')
  ...
  it('records the shipped `subagent_fork` alias in a note (config-driven tool name)', async () => {
    ...
    const catalog = await collectToolCatalog()
    const subagent = catalog.find(entry => entry.pkg === '@workspacealberta/wa-tool-subagent')
...
describe('gen-tool-catalog assertToolsHarvested', () => {
  const entry: ToolPackage = {
    pkg: '@workspacealberta/wa-tool-demo',
    dir: 'tool-demo',
  ...
  it('throws, naming the package and its requirements, when a boot registers nothing', () => {
    ...
    expect(() => { assertToolsHarvested(entry, 0) }).toThrow(/@workspacealberta\/wa-tool-demo booted without registering a single tool/)
...
describe('gen-tool-catalog render', () => {
  it('emits a package heading, a tool heading, and a json schema fence', () => {
    ...
    expect(md).toContain('| `@workspacealberta/wa-tool-demo` | `demo` | `ctx.tools` | `tool/result` |')
    expect(md).toContain('## `@workspacealberta/wa-tool-demo`')
