...
import * as SessionLogDownload from '@workspacealberta/wa-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@workspacealberta/wa-commands'",
      "- name: '@workspacealberta/wa-session-log-export'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@workspacealberta/wa-commands', CommandRuntime],
      ['@workspacealberta/wa-session-log-export', SessionLogDownload],
    ])
