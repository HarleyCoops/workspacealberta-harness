...
vi.mock('@workspacealberta/wa-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@workspacealberta/wa-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@workspacealberta/wa-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
