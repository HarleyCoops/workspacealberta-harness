 ...
 * @param name - the profile name (`wa --profile <name>`).
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@workspacealberta/wa-base', '@workspacealberta/wa-web-app'],
  headless: ['@workspacealberta/wa-base', '@workspacealberta/wa-headless'],
}
...
/** The bundle list a `wa plugin` init uses for a name with no shipped template. */
...
export function loadProfile(
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'wa plugin --profile ${name} add <package>'`,
      )
    }
