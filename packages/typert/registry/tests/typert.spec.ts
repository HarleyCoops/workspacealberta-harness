...
} from '@workspacealberta/wa-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@workspacealberta/wa-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@workspacealberta/wa-tools', 'ToolInput')).toBe('@workspacealberta/wa-tools#ToolInput')
    expect(typertPackageKey('@workspacealberta/wa-tools', 'host')).toBe('@workspacealberta/wa-tools#host')
    expect(ctx.typert.get('@workspacealberta/wa-tools#ToolInput')).toMatchObject({
      package: '@workspacealberta/wa-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@workspacealberta/wa-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@workspacealberta/wa-tools', 'host')).toMatchObject({
      key: '@workspacealberta/wa-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@workspacealberta/wa-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@workspacealberta/wa-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@workspacealberta/wa-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@workspacealberta/wa-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@workspacealberta/wa-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@workspacealberta/wa-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@workspacealberta/wa-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@workspacealberta/wa-tools#ToolInput')).not.toBe(projected)
  })
