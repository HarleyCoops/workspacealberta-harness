...
import type { TypertContribution } from '@workspacealberta/wa-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@workspacealberta/wa-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@workspacealberta/wa-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@workspacealberta/wa-tools')
    ...
    const record = ctx.typert.getPackage('@workspacealberta/wa-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@workspacealberta/wa-tools', 'host')).toBeUndefined()
  })
})
