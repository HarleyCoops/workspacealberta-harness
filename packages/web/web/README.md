...
| Package | Role |
|---|---|
| `@workspacealberta/wa-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@workspacealberta/wa-web-search-exa` | Search provider: Exa |
| `@workspacealberta/wa-web-search-perplexity` | Search provider: Perplexity |
| `@workspacealberta/wa-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@workspacealberta/wa-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
