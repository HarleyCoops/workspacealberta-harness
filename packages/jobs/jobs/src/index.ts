 ...
 * process-local registry lives in `@workspacealberta/wa-jobs-local`.
 * @module @workspacealberta/wa-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@workspacealberta/wa-jobs is the abstract job registry seam; load an implementation such as @workspacealberta/wa-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
