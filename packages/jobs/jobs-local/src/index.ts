 ...
 * @module @workspacealberta/wa-jobs-local
 */

import { Context } from '@workspacealberta/cordis'
import z from '@workspacealberta/schemastery'
import type { Agent } from '@workspacealberta/wa-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@workspacealberta/wa-scope'
import type { ScopeLayer } from '@workspacealberta/wa-scope'
import { deadline, timeoutOf } from '@workspacealberta/wa-timeout'
import { JobRegistry, JobId } from '@workspacealberta/wa-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@workspacealberta/wa-jobs'
 ...
 * `@workspacealberta/wa-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @workspacealberta/wa-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @workspacealberta/wa-agent)')
    }
