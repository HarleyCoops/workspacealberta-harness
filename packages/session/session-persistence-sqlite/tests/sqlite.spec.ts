...
import SessionStore, { SessionId, type SessionEvent } from '@workspacealberta/wa-session'
import SessionPersistenceSqlite, {
  DEFAULT_BUSY_TIMEOUT_MS,
  SCHEMA_VERSION,
} from '@workspacealberta/wa-session-persistence-sqlite'
import {
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@workspacealberta/wa-session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@workspacealberta/wa-session') return SessionStore
        if (specifier === '@workspacealberta/wa-session-persistence-sqlite') {
