...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@workspacealberta/wa-llm'
...
import * as providerPlugin from '@workspacealberta/wa-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@workspacealberta/wa-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@workspacealberta/wa-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@workspacealberta/wa-session-title', SessionTitleService],
    ['@workspacealberta/wa-session-title-first-prompt-llm', providerPlugin],
  ])
