...
npm scope: `@workspacealberta/wa-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@workspacealberta/wa-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`bundle/`](bundle/README.md) | Installable `wa --profile` patch layers | Product — stable API |
