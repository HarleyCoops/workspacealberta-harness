...
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@workspacealberta/wa-agent'
...
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@workspacealberta/wa-llm'
import { errorChain } from '@workspacealberta/wa-llm'
import type { ContentBlock, MessageSource } from '@workspacealberta/wa-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@workspacealberta/wa-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@workspacealberta/wa-session'
import type { SessionPersistence } from '@workspacealberta/wa-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@workspacealberta/wa-session-query'
import { SubagentError } from '@workspacealberta/wa-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@workspacealberta/wa-subagent'
import { isUserInvocable } from '@workspacealberta/wa-skill'
import type { Workspace, WorkspaceRecord } from '@workspacealberta/wa-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@workspacealberta/wa-workspace'
...
import type { PresetBearingSession } from '@workspacealberta/wa-agent-presets'
...
import type { SessionRawArtifact } from '@workspacealberta/wa-session-persistence'
import {
...
import type {} from '@workspacealberta/wa-jobs'
import type { JobSnapshot } from '@workspacealberta/wa-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@workspacealberta/wa-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@workspacealberta/wa-goal'
import type { GoalRef as CoreGoalRef } from '@workspacealberta/wa-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@workspacealberta/wa-commands'
...
import { SettingsConflictError, settingsNamespace } from '@workspacealberta/wa-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@workspacealberta/wa-settings'
import { credentialRef } from '@workspacealberta/wa-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@workspacealberta/wa-session-title'
import type { CallId } from '@workspacealberta/wa-llm/brand'
import type { ScopeKey } from '@workspacealberta/wa-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@workspacealberta/wa-user-approval'
...
} from '@workspacealberta/wa-user-questions'
import { UserQuestionError } from '@workspacealberta/wa-user-questions'
import { DirectoryPickerError } from '@workspacealberta/wa-host-directory-picker'
import {
...
} from '@workspacealberta/wa-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @workspacealberta/wa-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @workspacealberta/wa-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @workspacealberta/wa-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @workspacealberta/wa-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @workspacealberta/wa-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @workspacealberta/wa-skill', details: {} })
        }
