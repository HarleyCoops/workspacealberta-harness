...
describe('createFixtureApi', () => {
  ...
  it('serves grouped models and keeps a selection for later history and fixture requests', async () => {
    ...
    expect(catalog.result.value.groups.map(group => group.name)).toEqual(['Cohere (Canada)', 'OpenAI'])
    expect(catalog.result.value.groups[0]?.models.map(model => model.id))
      .toEqual(['command-a-plus-05-2026'])
  ...
  it('serves configured Cohere readiness and keeps credential values write-only', async () => {
    const api = createFixtureApi()
    ...
    const initial = await api.credentials.describe(req({ refs: ['COHERE_API_KEY', 'TEST_API_KEY'] }))
    if (!initial.result.ok) throw new Error('credential describe failed')
    expect(initial.result.value.credentials).toEqual({
      COHERE_API_KEY: { configured: true, source: 'file', writable: true },
