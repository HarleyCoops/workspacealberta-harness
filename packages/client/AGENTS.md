...
Packages here are named with the directory prefix: `@workspacealberta/wa-client-<name>`.
...
2. **A dynamic package declares internal dynamic relationships as peer plus dev.** Production source imports, re-exports, module augmentations, and type-only references to an `@workspacealberta/wa-*` package count, as does a package named by `dsh.client.inject`. A test-only internal dependency stays dev-only.
...
5. Rebuild the bundle (`pnpm --filter <pkg> bundle`) before probing a live `wa web` server — the registry serves `lib/client.js`, not sources.
