...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@workspacealberta/wa-agent-spine-demo" in config
    assert "@workspacealberta/wa-session-persistence-jsonl" in config
    assert "@workspacealberta/wa-session-checkpoint-policy" in config
